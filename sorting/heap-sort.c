 ...
 * 	@author: Satyajith
 *
 * 	heap sort algorithm on max heaps
...
/**
 * swap variables at given locations
 *
...
/**
 * method to maintain the max-heap
 * property at a given node
 * - runs in O(lg n) time
 *
 * @param  A  heap array
...
/**
 * method to build the max-heap from an array
 * - runs in O(n) time
 * 
 * @param  A  heap array
 * @param  size  heap size
...
void heap_sort(int* arr, int size) {
	...
	for (int i = max_heap->heap_size; i >= 2; --i)
	{
		...
		(max_heap->heap_size)--;						// since an element is removed, size of heap reduces by 1
